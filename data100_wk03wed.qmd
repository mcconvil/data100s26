...
```{r}
game_counts <- count(buf, game_type)
game_counts
```
...
#| output-location: column
ggplot(data = game_counts,
       mapping = aes(x = fct_reorder(game_type, n),
                     y = n)) +
  geom_col()
```
...
```{r}
count(nfl_outcomes_2018, home_team, sort = TRUE)
```
...
buf %>%
  arrange(score_diff) %>%
  select(home_team, away_team, home_score, away_score, score_diff)
```
:::
