...
Code a Valentine: Tomorrow, 4:30pm, Taylor 212
...
## Data Setting: [Bureau of Labor Statistics (BLS) Consumer Expenditure Survey](https://www.bls.gov/cex/) {.smaller}
...
```{r}
# Note: These are fake DATA 100 students
students <- data.frame(name = c("Alice", "Josh", "Megan", "Eric", "Jean"),
                       res_hall = c("South Campus Apts", "Smith", "Off-campus", "Smith", "Trax"),
                       caf = c("like", "hate", "hate", "neutral", "love"))

halls <- data.frame(name = c("Kress", "Smith", "South Campus Apts", "Trax"),
                    steps = c(250, 450, 1400, 400))
```
...
students_new <- left_join(students, halls)
students_new
```
...
```{r}
students_new <- left_join(students, halls, join_by("res_hall" == "name"))
students_new
```
...
```{r}
students_new <- inner_join(students, halls, join_by("res_hall" == "name"))
students_new
```
...
```{r}
students_new <- full_join(students, halls, join_by("res_hall" == "name"))
students_new
```
...
Should I name my new dataframe `ce` or `ce1`? In other words, should I write over my original dataframe or should I save a new dataframe?
